...
if strcmp(data, 'legend')
    ...
    elseif length(time) == 3
        ...
        for f = 1:numOfStimuli
            ...
            text(1.2, f, ['Condition ' num2str(f)])
        end
    ...
    elseif length(time) == 4 && time(3) == 2
        ...
        for f = 1:numOfStimuli
            plot([0.5 1], [f f], 'color', colors(f,:), 'LineWidth', 2)
            text(1.2, f, ['Direction ' num2str(f)])
        end
...
elseif ndims(data) == 3
    ...
    plot(time, squeeze(data(1,1,:)),"color",[0 0.4470 0.7410],'LineWidth', 2)
    plot(time, squeeze(data(1,2,:)),"color",[ 0.85 0.325 0.098],'LineWidth', 2)   
    plot(time, squeeze(data(1,3,:)),"color",[0.929 0.694 0.125],'LineWidth', 2)   
    plot(time, squeeze(data(1,4,:)),"color",[0.494 0.184 0.556],'LineWidth', 2)   
